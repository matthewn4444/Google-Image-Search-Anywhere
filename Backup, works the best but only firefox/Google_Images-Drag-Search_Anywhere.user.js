// ==UserScript==
// @name        Google Images-Drag-Search Anywhere
// @namespace   matthewn4444
// @description Dragging any image allows you to do a google image search.
// @include     *
// @exclude     *www.google.ca/imghp*
// @exclude     *www.google.ca/search?*site=imghp&*
// @exclude     *www.google.com/search?tbs=sbi:*
// @version     1
// ==/UserScript==

if (!document || !document.body || top !== self) return;

//after move image out, it doesnt close 
...
function setImageForm(dataUrl, fileName) {
    ...
    EL.form = createEl("form", "userscripts-form-image-search", {
                    ...
                    method:     'POST',
                    target:     '_blank',
                    action:     'http://www.google.ca/searchbyimage/upload'
                });
...
function detachDragEvents() {
    ...
    document.addEventListener("dragleave", checkLeftBrowser, false);
    EL.zone.addEventListener("dragover", stopBrowser, false);
...
function attachDragEvents(shouldRemoveEvents) {
    ...
    document.addEventListener("dragexit", a, false);
    
    if (!shouldRemoveEvents) {
        document.removeEventListener("dragleave", checkLeftBrowser, false);
        EL.zone.removeEventListener("dragover", stopBrowser, false);
...
function checkLeftBrowser(event) {
    console.log(event.target);
    var ele = event.relatedTarget;
    try{
        // Check to see if the element is accessable
        if (ele != null && ele.tagName != "HTML") {
            return;
        }
    }catch(error){}
    hide();
